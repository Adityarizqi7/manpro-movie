        ...
        blog ? 
            (
                <Netray
                    ...
                    twitTitle={status}
                >
                    <main className={`${renderTheme(theme, "bg-dark-theme")} readblog-component`}>
                        <section id="container_readblog" className="md:w-[70%] w-full">
                            <div className="content-blog space-y-10 inter">
                                <div className="flex items-center">
                                    <div className='w-full flex items-center gap-y-4 flex-wrap'>
                                        <Link to={`/blogs/${slugCategory}`}>
                                            <span className='py-2 px-2 bg-gray-200 rounded-[5px]'>{blog?.category}</span>
                                        </Link>
                                        <span className={`${renderTheme(theme, "!text-neutral-100")} px-[8px]`}>·</span>
                                        <span className={`${renderTheme(theme, "!text-neutral-100")} font-medium`}>{blog?.authorName}</span>
                                        <span className={`${renderTheme(theme, "!text-neutral-100")} px-[8px]`}>·</span>
                                        <span className={`${renderTheme(theme, "!text-neutral-100")}`}>{blog?.readTime} min read</span>
                                    </div>
                                    <BtnSosmedShare 
                                    classBtnShare="w-full flex items-center justify-end" title={blog?.title} 
                                    fill="#CECDCD" size="21"/>
                                </div>
                                <h2 className={`${renderTheme(theme, "!text-neutral-100")} card-title`}>{blog?.title}</h2>
                                <img className="card-image object-cover 3xs:object-fill w-full aspect-square h-[28rem]" src={blog?.image} alt={blog?.title}/>
                                <div className="card-text">
                                    <p 
                                        dangerouslySetInnerHTML={{ __html: blog?.description }} className={`${renderTheme(theme, "!text-neutral-100")} whitespace-pre-line align-bottom leading-relaxed`}>
                                    </p>
                                </div>
                            </div>
                            <aside className="share-button mt-7 pt-12 px-2 space-y-10 border-t border-solid border-y-gray-300 inter">
                                <h2 className={`${renderTheme(theme, "!text-neutral-100")} text-center font-light leading-relaxed`}>Suka dengan konten ini? Yuk bagikan ke temanmu!</h2>
                                <BtnSosmedShare 
                                                    ...
                                                    return (
                                                        e.id === blog?.id ? false : (
                                                            <span key={i} className="wrapper-post flex flex-col group">
                                                                <Link to={`/blogs   /${e.slugCategory}/${e.slug}`}>
                                                                    <MiniCardBlog
                                                                        path={e.image} 
                                                                        text1={e.title} 
                                                                        text2={e.createdAt} 
